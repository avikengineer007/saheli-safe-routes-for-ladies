...
export interface SegmentData {
  ...
  isLit?: boolean;
  poiDensity?: number; // foot traffic / business density proxy (0.0 quiet to 1.0 vibrant)
  historicalIncidentsCount?: number; // police NCRB open data density count in 150m
  recentCrowdsourcedReports?: Array<{ ageDays: number; severity: number }>; // reports within 30 days
  sosFreeStreakCount?: number; // count of completed safe journeys without SOS
}
...
export interface SegmentScoreResult {
  ...
  breakdown: {
    ...
    crowdsourcedPenalty: number;
    lightingPenalty: number;
    isolationPenalty: number;
    sosStreakBonus: number;
    nightMultiplierUsed: number;
  };
  reasons: string[];
}

export class DeterministicSafetyScorer {
  /**
   * Generates a deterministic hash for a polyline segment
   */
  public static getSegmentHash(start: SegmentPoint, end: SegmentPoint): string {
   ...
   */
  public static getNightMultiplier(timestamp: Date = new Date()): number {
    const hour = timestamp.getHours();
    const { start, end } = scoringConfig.parameters.night_hours;
    const isNight = hour >= start || hour < end;
    return isNight ? scoringConfig.parameters.night_multiplier : scoringConfig.parameters.day_multiplier;
  }
   ...
   *       + w5 * sos_free_streak
   */
  public static scoreSegment(segment: SegmentData, evalTime: Date = new Date()): SegmentScoreResult {
    ...
    const halfLifeDays = scoringConfig.parameters.decay_half_life_days;
