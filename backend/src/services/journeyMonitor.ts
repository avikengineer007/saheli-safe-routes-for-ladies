...
export interface ActiveJourneyState {
  ...
  lastPing?: { lat: number; lng: number; timestamp: Date };
  contactAlertLogs: Array<{ timestamp: Date; type: string; message: string }>;
}

export class JourneyMonitorService {
  /**
  ...
  public static pointToSegmentDistance(
    ...
    const dAB = RoutingService.haversineDistance(a, b);
    if (dAB === 0) return RoutingService.haversineDistance(p, a);
    ...
    return RoutingService.haversineDistance(p, projection);
  }

  /**
   * Distance from point P to polyline
   */
  public static minDistanceToPolyline(
   ...
   */
  public static processLocationPing(
    journey: ActiveJourneyState,
    lat: number,
    lng: number
  ): { onRoute: boolean; alertTriggered: boolean; alertMessage?: string } {
    const distToPolyline = this.minDistanceToPolyline({ lat, lng }, journey.routePolyline);
    const thresholdMeters = scoringConfig.parameters.deviation_threshold_meters; // 150m
    const maxConsecutivePings = scoringConfig.parameters.deviation_consecutive_pings; // 2 pings
   ...
   */
  public static checkETAExpiry(journey: ActiveJourneyState): { isExpired: boolean; alertMessage?: string } {
    if (journey.status !== 'active') return { isExpired: false };

    const now = new Date().getTime();
    const startTime = new Date(journey.startedAt).getTime();
    const etaMs = journey.etaMinutes * 60 * 1000;
    const bufferMs = scoringConfig.parameters.eta_buffer_minutes * 60 * 1000;
   ...
   */
  public static async dispatchSMS(phone: string, message: string): Promise<{ success: boolean; data?: any; error?: string }> {
