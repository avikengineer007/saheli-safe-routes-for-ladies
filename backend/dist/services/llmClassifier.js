...
exports.LLMAuxiliaryService = void 0;
const sdk_1 = __importDefault(require("@anthropic-ai/sdk"));
class LLMAuxiliaryService {
    static getClient() {
        const key = process.env.ANTHROPIC_API_KEY;
        return key ? new sdk_1.default({ apiKey: key }) : null;
    }
    /**
     ...
     */
    static async classifyIncidentDescription(description) {
...
Respond ONLY in JSON format: {"category": string, "severityAuto": string, "confidence": number, "reasoning": string}`,
     ...
     */
    static generatePlainLanguageExplanation(reasons) {
        if (reasons.length === 0) {
    ...
    static fallbackLocalTriage(text) {
        const lower = text.toLowerCase();
        let category = 'other';
        let severityAuto = 'low';
...
exports.LLMAuxiliaryService = LLMAuxiliaryService;
