...
class RoutingService {
    ...
    static async fetchGoogleDirections(origin, dest) {
    ...
    static async calculateSafeRoutes(origin, dest, maxDetourBudgetPercent = scoringConfig_json_1.default.parameters.max_detour_budget_percentage) {
        ...
        for (let i = 0; i < candidatePaths.length; i++) {
            ...
            let totalLength = 0;
            const aggregatedReasonsSet = new Set();
            for (let j = 0; j < candidate.polyline.length - 1; j++) {
                ...
                const scoreRes = safetyScorer_1.DeterministicSafetyScorer.scoreSegment(envData);
        ...
        const validRoutes = scoredCandidates.filter(r => r.distanceMeters <= maxDistanceAllowed || r.tag === 'fastest');
    ...
    static resolveLocation(input, defaultName) {
    ...
    static generateKolkataCandidatePolylines(origin, dest) {
    ...
    static calculatePolylineDistance(polyline) {
        let total = 0;
        for (let i = 0; i < polyline.length - 1; i++) {
            total += this.haversineDistance({ lat: polyline[i][0], lng: polyline[i][1] }, { lat: polyline[i + 1][0], lng: polyline[i + 1][1] });
        }
        return total;
    }
}
