...
const journeyMonitor_1 = require("../services/journeyMonitor");
...
const activeJourneysMap = new Map();
/**
 ...
 */
exports.apiRouter.post('/routes/plan', async (req, res) => {
    try {
        const { origin, destination, originName, destName, maxDetourBudgetPercent } = req.body;
        const resolvedOrigin = routingService_1.RoutingService.resolveLocation(origin || originName, originName || 'Park Street Metro');
        const resolvedDest = routingService_1.RoutingService.resolveLocation(destination || destName, destName || 'Rabindra Sadan');
        const result = await routingService_1.RoutingService.calculateSafeRoutes(resolvedOrigin, resolvedDest, maxDetourBudgetPercent || 25);
        res.json(result);
    }
    catch (err) {
 ...
 */
exports.apiRouter.post('/journey/start', (req, res) => {
    try {
 ...
 */
exports.apiRouter.post('/journey/ping', (req, res) => {
    try {
        ...
        const pingRes = journeyMonitor_1.JourneyMonitorService.processLocationPing(journey, lat, lng);
        const etaRes = journeyMonitor_1.JourneyMonitorService.checkETAExpiry(journey);
        res.json({
            journeyId,
 ...
 */
exports.apiRouter.post('/journey/sos', (req, res) => {
    try {
        ...
        const sosPayload = journeyMonitor_1.JourneyMonitorService.triggerSOS(journey, userName || 'SAHELI User', currentLocation || { lat: 22.5552, lng: 88.3510 }, contactPhone);
        res.json({
            success: true,
 ...
 */
exports.apiRouter.post('/incidents/report', async (req, res) => {
    try {
        ...
        const result = await incidentService_1.IncidentService.submitReport({
 ...
 */
exports.apiRouter.get('/incidents/heatmap', (req, res) => {
    try {
        const points = incidentService_1.IncidentService.getPublicHeatmapPoints();
        res.json({
